...
interface UseUploadFileProps {
  ...
  onUploadBegin?: UploadFilesOptions<
    OurFileRouter,
    keyof OurFileRouter
  >["onUploadBegin"];
  onUploadProgress?: UploadFilesOptions<
    OurFileRouter,
    keyof OurFileRouter
  >["onUploadProgress"];
  headers?: UploadFilesOptions<OurFileRouter, keyof OurFileRouter>["headers"];
  skipPolling?: UploadFilesOptions<
    OurFileRouter,
    keyof OurFileRouter
  >["skipPolling"];
  category: string;
...
export function useUploadFile(
  ...
  async function onUpload(files: File[]) {
    ...
    if (maxFileCount && files.length > maxFileCount) {
      toast({
        title: "Error",
        description: `You can only upload up to ${maxFileCount} files.`,
        variant: "destructive",
      });
      return;
    }
